...
//---------------------- Compute Prayer Times -----------------------
		...
		def computeDayTimes(self):
			...
			for i in xrange( 1, self.numIterations):
		...
		def adjustTimes(self, times):
			...
			if self.calcParams[self.calcMethod][1] == 1: # Maghrib
				times[5] = times[4] + self.calcParams[self.calcMethod][2]/ 60
			if (self.calcParams[self.calcMethod][3] == 1) # Isha
				times[6] = times[5] + self.calcParams[self.calcMethod][4]/ 60
		...
		def adjustTimesFormat(self, times):
			...
			for i in xrange(0,7):
				...
				else if self.timeFormat == PT_TIME_12NS:
		...
		def adjustHighLatTimes(self, times):
			...
			if not times[0].isdigit() or timeDiff(times[0], times[1]) > FajrDiff:
				times[0] = times[1]- FajrDiff

			// Adjust Isha
			IshaAngle = (self.calcParams[self.calcMethod][3] == 0) ? self.calcParams[self.calcMethod][4] : 18
			IshaDiff = self.nightPortion(IshaAngle)* nightTime
			if not times[6].isdigit() or timeDiff(times[4], times[6]) > IshaDiff:
				times[6] = times[4]+ IshaDiff

			// Adjust Maghrib
			MaghribAngle = (self.calcParams[self.calcMethod][1] == 0) ? self.calcParams[self.calcMethod][2] : 4
			MaghribDiff = self.nightPortion(MaghribAngle)* nightTime
			if times[5].isdigit() or timeDiff(times[4], times[5]) > MaghribDiff:
...
// http://aa.usno.navy.mil/faq/docs/SunApprox.html
		...
		def computeMidDay(self, t)
			var T = equationOfTime(self.JDate + t)
			var Z = fixhour(12 - T)
			return Z

		// compute time for a given angle G
		def computeTime(self, G, t)
			var D = sunDeclination(self.JDate+ t)
			var Z = self.computeMidDay(t);
			var V = 1/15* darccos((-dsin(G)- dsin(D)* dsin(self.lat))/
		...
		def computeAsr(self, step, t):  // Shafii: step=1, Hanafi: step=2
			var D = sunDeclination(self.JDate+ t)
			var G = -darccot(step+ dtan(Math.abs(self.lat- D)))
...
def fixangle(angle):
	return round( fmod( angle, 360.0 ), 2 )

// range reduce hours to 0..23
def fixhour(hour):
	return round( fmod( hour, 24.0 ), 2 )


//---------------------- prayTime Object -----------------------

def prayTime = PrayTime();
